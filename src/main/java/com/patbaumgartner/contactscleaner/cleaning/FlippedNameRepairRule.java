 ...
 *
 * <p>
 * Example: the card says given = {@code Muster}, family = {@code Max}, and the contact's
 * e-mail is {@code max.muster@example.com}. The local part matches <em>family.given</em>
 * ({@code max.muster}) but not <em>given.family</em> ({@code muster.max}) — unambiguous
...
final class FlippedNameRepairRule implements VCardCleaningRule {
	...
	/** Lower-case and fold accents so 'García' matches 'garcia' in an address. */
